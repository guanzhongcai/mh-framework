...
module.exports = function (req, res, next) {
    ...
    //时间戳检查
    if (!__timestamp) {
        return send(HttpStatus.Bad_Request, "时间戳字段缺失");
    } else {
        const diff = Math.abs(new Date().getTime() - __timestamp);
        if (diff > MAX_DIFF_TIME * 1000) {
            return send(HttpStatus.Bad_Request, `过期的客户端请求！(${diff / 1000}秒前的请求)`);
        }
    }

    //签名检查
    if (!__sign) {
        return send(HttpStatus.Token_Required, "签名字段缺失");
    }
    ...
    if (__sign === sign) {
        ...
        return send(HttpStatus.Unauthorized, "错误的签名");
    }
};
