...
async function TaskGetAwards(request, response)
{
	...
	if(_.isEmpty(taskNode)){
		...
		let taskStatus =  await request.taskController.checkTaskStatValid(request.body.uuid, request.body.taskId);
		if(!taskStatus){
...
//成就具体具体详情
async function achievementData(request, response){
	
	var respData = protocol.responseData(request.body.httpuuid, request.body.uuid);
	let {sList,rList }= await request.taskController.getAchievementData(request.body.uuid)
	// 判断是否已领取
	request.multiController.save(async function(err,data){
		if(err){
			respData.code = ERRCODES().FAILED;
		...
		respData.taskEventData = [];
		respData.taskList = [];
		respData.achievementScores = sList;
		respData.achievementRewarded = rList;
		protocol.responseSend(response, respData);
	})
...
async function achievementReward(request, response){
	...
	let isExist = await request.taskController.achievementHashGetById(request.body.uuid, aId)
	if(isExist){
	...
	respData.achievementData = {id:aId, time: (new Date().getTime())}
	request.multiController.save(async function(err, data){
		respData.taskEventData = [];
		respData.taskList = [];
		try {
