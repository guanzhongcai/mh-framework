...
class RedisHelper
{
	...
	setValueExist(tblname, field, callback)
	{
		this.redisClient.SISMEMBER(tblname, field, (err, result) => {
	...
	setAdd(tblname, field, callback)
	{
		

		this.redisClient.SADD(tblname, field, (err, result) => {
			if (err) { console.error(err); }
			callback(result);
		});
	}
	...
	async achievementExist(uuid,tableName,data)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setValueExist(redis_key,data,function (res) {
				resolve(res)
			})
		})
	}
	...
	async taskGetAll(uuid,tableName,data)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setHash(redis_key,data,function (data) {
	...
	//refreshData
	async getRefreshDataByType(uuid, tblName, action)
	{
		return await this.singleGet(uuid,tblName,action)
	}
	
	async setDailyTaskRefreshTimes(uuid,tableName,rdata)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setHash(redis_key,rdata,function (data) {
				resolve(data)
			})
		})
	}
	...
	//mainTask
	async getOpenTaskList(uuid, tableName)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.getString(redis_key,function (data) {
				resolve(data)
			})
		})
	}
	
	async setOpenTaskList(uuid,tableName, sdata)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setString(redis_key,sdata,function (data) {
				resolve(data)
			})
		})
	}
	...
	async achievementScoreListGet(uuid,tableName)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.getString(redis_key,function (data) {
	...
	async achievementScoreSET(uuid,tableName, sdata)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setString(redis_key,sdata,function (data) {
	...
	async achievementHashGetById(uuid,tableName,aid)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.getHashFieldValue2(redis_key, aid,function (data) {
				resolve(data)
			})
		})
	}
	
	async achievementHashSet(uuid,tableName,aid,times)
	{
		let  redis_key = tableName+':' + uuid
		return new Promise(resolve => {
			this.setHashFieldValue2(redis_key, aid,times,function (data) {
				resolve(data)
			})
		})
	}
	...
	async taskCheck(uuid,tableName)
	{
		return new Promise(resolve => {
			this.exist(tableName,uuid,function (err, data) {
				resolve(data)
			})
		})
	}
	...
	async achievementGetData(...actions)
	{
		// 示例：[18808031, 12,13,14]
		let uuid = actions.shift()
		let tblName = actions.shift()
		this.s(tableName,uuid,function (err, data) {
			resolve(data)
		})
	...
	async taskGetCountData(...actions)
	{
		// 示例：[18808031, 12,13,14]
		let uuid = actions.shift()
		let tblName = actions.shift()
