...
class RedisHelper
{
	constructor(redisCfg)
	{
		...
		this.redisOuthSyncTableName  = "OuthData" + this.redisSyncTableName;
		...
		if (tblname == 'GlobalData') {
			...
			this.redisClient.set(this.getKey(tblname, field), value, (err, result) => {
				...
				if (tblname != this.redisSyncTableName && 'number' == typeof field && field > 18808029) {
					...
					if (tblname == "OuthData") {
						this.updateUserOuthData (tblname, field)
					}
				}
				callback(result);
			});
	...
	//update user outh data ..
	updateUserOuthData (tblname, field)
	{
		this.setHashFieldValue2 (this.redisOuthSyncTableName, field,  JSON.stringify({}),  () => {
		});
	}
